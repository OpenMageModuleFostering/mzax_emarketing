...
class Symfony_Component_CssSelector_Parser_Shortcut_HashParser implements Symfony_Component_CssSelector_Parser_ParserInterface
{
    ...
    public function parse($source)
    {
        ...
        if (preg_match('/^(([a-z]+)\|)?([\w-]+|\*)?#([\w-]+)$/i', trim($source), $matches)) {
            return array(
                new Symfony_Component_CssSelector_Node_SelectorNode(new Symfony_Component_CssSelector_Node_HashNode(new Symfony_Component_CssSelector_Node_ElementNode($matches[2] ?: null, $matches[3] ?: null), $matches[4])),
            );
        }
